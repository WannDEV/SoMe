...
export const createPost = async (req, res) => {
    try {
        ...
        const query = 'INSERT INTO post (userID, content, img) VALUES ($1, $2, $3) RETURNING *';
...
export const getPosts = async (req, res) => {
    try {
        ...
        const friendsQuery = 'SELECT friendID FROM friendships WHERE userID = $1 AND status = $2';
        ...
        const postsQuery = 'SELECT * FROM post WHERE userID = ANY($1::int[]) ORDER BY postDate DESC';
...
export const updatePost = async (req, res) => {
    try {
        const postId = req.params.postId;
        const { userId, content, img } = req.body;
        // Check hvis brugeren der udfører handlingen, er ejeren af opslaget
        const ownershipQuery = 'SELECT * FROM post WHERE postID = $1 AND userID = $2';
        ...
        const updateQuery = 'UPDATE post SET content = $1, img = $2 WHERE postID = $3 RETURNING *';
...
export const deletePost = async (req, res) => {
    try {
        ...
        // heck hvis brugeren der udfører handlingen, er ejeren af opslaget
        const ownershipQuery = 'SELECT * FROM post WHERE postid = $1 AND userid = $2';
        const ownershipResult = await pool.query(ownershipQuery, [postId, userId]);
        if (ownershipResult.rows.length === 0) {
